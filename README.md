...
5. 운영 결제 결과는 AES-256-GCM으로 암호화한 뒤 Supabase에 저장하고, 복구 코드는
   해시만 보관합니다.
6. 결제 상태 웹훅은 토스 API에서 결제를 다시 조회한 뒤 저장 상태를 갱신합니다.
   취소·부분취소된 주문은 복구 코드와 내 서재에서 다시 열리지 않습니다.
...
토스페이먼츠 개발자센터에서 `PAYMENT_STATUS_CHANGED` 이벤트 URL을 아래와 같이
등록합니다.

```text
https://운영도메인/api/webhooks/toss
```

일반 결제 웹훅에는 검증 서명이 없으므로, 이 엔드포인트는 본문의 상태를 신뢰하지
않고 결제 키로 토스 결제 조회 API를 호출해 주문번호·금액·현재 상태를 재검증합니다.
